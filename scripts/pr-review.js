...
const MODEL_NAME = "gemma-4-31b-it"; // Note: Check if this model exists/is correct
...
function loadPrompts(mode, userQuery = '') {
    ...
    try {
        ...
        return `Return JSON only. Mode: ${mode}.`;
    }
}
...
async function analyzeWithGemini(diffData, priorityFilesContext, mode, userQuery = '') {
    ...
    const mergedPrompt = `${systemPrompt}\n\nCRITICAL: Return ONLY JSON. If you explain or think out loud, I will fail.\n\nPR Diff:\n${diffData}\n\nContext:\n${priorityFilesContext}`;
    ...
    const extractJSON = (str) => {
        ...
        if (first !== -1 && last !== -1) {
            try { return JSON.parse(str.substring(first, last + 1)); }
            catch (e) { return null; }
        }
        return null;
    };
    ...
    try {
        ...
        if (mode === 'test') {
            ...
            if (created.length > 0) {
                ...
                runCommand(`git commit -m "test: add unit tests"`);
                ...
                await octokit.issues.createComment({ owner, repo, issue_number: PR_NUMBER, body: "❌ **AI Assistant**: No tests were generated. Check Action logs for raw output." });
            }
        } else {
            ...
            const comments = reviewItems.map(item => {
                ...
                return { path: item.file, line: parseInt(item.line), side: 'RIGHT', body: `${item.comment}${item.suggestion ? `\n\n\`\`\`suggestion\n${item.suggestion}\n\`\`\`` : ''}` };
